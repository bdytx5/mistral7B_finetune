...
text = "<s>[INST] Make a flask API that will host a simple hello world website. it should show the text on the screen using html[/INST]"
...
model, peft_config, tokenizer = create_and_prepare_model(script_args)
ft_model = PeftModel.from_pretrained(model, "/root/results_packing/checkpoint-150")
...
with torch.no_grad():
    ...
    print("#"*50)
    print(tokenizer.decode(ft_model.generate(**model_input, max_new_tokens=512, pad_token_id=2)[0], skip_special_tokens=True).split("</s>")[0])
